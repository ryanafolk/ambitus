...
At the very end of an `ambitus` run, it will ask whether to remove temporary files. These will need to be kept if you plan to run the downstream analyses provided in the postprocessing directory, but they are not needed for the standalone code "ambitus-postprocessor."
